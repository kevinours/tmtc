...
export class DatabaseService {
      ...
      if (data.rows.length > 0) {
        for (var i = 0; i < data.rows.length; i++) {
          ...
          developers.push({ 
            ...
            skills: skills, 
            postDate: data.rows.item(i).postDate,
  ...
  loadDevelopersBy(type) {
    return this.database.executeSql('SELECT * FROM developer ORDER by '+type, []).then(data => {
      let developers: Dev[] = [];
  ...
  addDeveloper(name, skills, img, postDate) {
    let data = [name, JSON.stringify(skills), img, 0, postDate];
    return this.database.executeSql('INSERT INTO developer (name, skills, img, score, postDate) VALUES (?, ?, ?, ?, ?)', data).then(data => {
      this.loadDevelopers();
    });
  }
  ...
  getDeveloper(id): Promise<Dev> {
    return this.database.executeSql('SELECT * FROM developer WHERE id = ?', [id]).then(data => {
      ...
      return {
        ...
        postDate : data.rows.item(0).postDate,
        skills: skills, 
  ...
  upScore(id, score) {
    let data = [score+1];
    return this.database.executeSql(`UPDATE developer SET score = ? WHERE id = ${id}`, data).then(data => {
        this.loadProducts();
    });
  }
